	...
	private void check()
	{
		...
		try {
			if (charButton.isInside(lastX, lastY)) {
				...
				item = collection.remove();		//		and we print it out
				System.out.println("  --> " + item);

				theApplet.showStatus("  --> " + item);
			}
			else if (reverseButton.isInside(lastX, lastY)) {
				collection.reverse();			//	Exchange the first two elements
			}
			else  {
...
//
	...
	private List collection;			//	To hold our elements

	private Abutton charButton,			//	To push a character element
					intButton,			//		or an integer element,
					removeButton,		//		to pop an element
					reverseButton;		//		to exchange the top two elements
