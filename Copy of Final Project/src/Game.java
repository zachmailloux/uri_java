...
public class Game extends Frame implements MouseListener, ActionListener {
	...
	private int totalducks = 500;  //the total amount of ducks in the array
	...
	public void addingducks(){ 
		for (int i = 0; i < totalducks; i++){ /**for in the total amount of ducks*/
			...
			if(i>=250){
				direction = 1;}
			Duckfield[i] = new Duck(randX,randY, direction,true); //this makes the ducks, rare ducks and blood field on the SAME COORDINATE LIST
		...
		for (int i = 0; i < goldducks; i++){
			int randX = random.nextInt(500)+200; /**create */
			int randY = random.nextInt(150)+400;
			if(i<5){ //this loops causes half of the ducks to face left and half to face right.
				direction = 0;}
			if(i>=5){
				direction = 1;}
	...
	public void mousePressed(MouseEvent e) {
		...
		int y = e.getY();
		for (int i = 0; i < goldducks;i++){
			if ((GoldDuckfield[i].isInside(x, y)) && (GoldDuckfield[i].visible == true)&&(menuvalue == false)){
				GoldDuckfield[i].visible = false;
				if(time<=35){
				time = time +5;}
			}
		}
	...
	public void paint(Graphics pane){
		...
		for(int i=0; i<goldducks; i++){ //if the duck is registered as visible, print it
			if(GoldDuckfield[i].isVisible() == true){
				GoldDuckfield[i].fly(pane);}
		}
